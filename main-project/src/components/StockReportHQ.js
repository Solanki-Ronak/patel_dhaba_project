...
const StockReporthq = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let stockCollectionRef = collection(firestore, 'hq_products_purchased');
        ...
        const total = data.reduce((acc, item) => acc + parseInt(item.quantity), 0); // Convert to integer
        setTotalPurchased(total);

        // Fetch total sold from Firebase
        const salesCollectionRef = collection(firestore, 'sales_hq');
        ...
        const soldTotal = soldData.reduce((acc, item) => {
          if (productCode === '' || productCode === item.productCode) {
            return acc + parseInt(item.quantity); // Convert to integer
          }
          return acc;
        }, 0);
  ...
  return (
    <div className="stock-report-container">
      ...
      <h3>Stock Report Headquarter</h3>
