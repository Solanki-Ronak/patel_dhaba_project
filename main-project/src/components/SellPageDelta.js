...
function SellPage() {
  ...
  const handleSell = async (e) => {
    ...
    try {
      const productQuery = query(collection(firestore, 'beta_products_remaining_new'), where('productCode', '==', productCode));
  ...
  const addSaleToHQ = async () => {
    ...
    try {
      await addDoc(collection(firestore, 'sales_beta'), saleData);
    } catch (error) {
